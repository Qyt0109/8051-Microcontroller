...
Mode 0 ung dung mo rong ngo ra:
			8051	 				Shift Register (VD: 74595)
								 - - - - - - - - - - -
			TXD	----->|	Clock (SH_CP)				|----->		8
			RXD	----->|	Data	(DS)					|----->	Extra
UART_ST_CP----->| Xuat du lieu (ST_CP)|. . .	Output
								|											|---->
								 - - - - - - - - - - - 
...
void UART0_Write(unsigned char UART_Byte)
{
	SBUF = UART_Byte;
	while(TI == 0);
	TI = 0;
